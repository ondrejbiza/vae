...
class GM_VAE:
    ...
    def __init__(self, num_inputs, encoder_neurons, decoder_neurons, cluster_predictor_neurons,
                 num_clusters, x_size, w_size, loss_type, weight_decay, learning_rate,
                 cluster_predictor_activation=tf.nn.tanh, clip_z_prior=None):
    ...
    def build_placeholders(self):
        ...
        self.input_pl = tf.placeholder(tf.float32, shape=(None, self.num_inputs), name="input_pl")
    ...
    def build_network(self):
        ...
        with tf.variable_scope(self.MODEL_NAMESPACE):
            ...
            # decoder
            self.logits_t, self.output_t = self.build_decoder(self.x_sample_t)
    ...
    def build_middle(self, input_1_t, share_weights=False):
        ...
        with tf.variable_scope("middle", reuse=share_weights):
            ...
            z_pred_exp -= tf.reduce_max(z_pred_exp)
    ...
    def build_decoder(self, input_t, share_weights=False):
        ...
        with tf.variable_scope("decoder", reuse=share_weights):
            ...
            for idx, neurons in enumerate(self.decoder_neurons):
                ...
                last = idx == len(self.decoder_neurons) - 1

                with tf.variable_scope("fc{:d}".format(idx + 1)):
                    x = tf.layers.dense(
                        x, neurons, activation=tf.nn.relu if not last else None,
                        kernel_regularizer=utils.get_weight_regularizer(self.weight_decay),
                        kernel_initializer=tf.contrib.layers.xavier_initializer()
                    )
                ...
                self.output_loss_t = tf.reduce_mean(
                    tf.reduce_sum(
                        tf.losses.sigmoid_cross_entropy(
                            multi_class_labels=self.input_pl, logits=self.logits_t,
                ...
                self.output_loss_t = tf.reduce_mean(
                    tf.reduce_sum(
                        tf.losses.mean_squared_error(
                            labels=self.input_pl, predictions=self.logits_t,
                            reduction=tf.losses.Reduction.NONE
                        ),
                        axis=1
                    ),
